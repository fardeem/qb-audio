...
function AudioPlayer({ src, title }: AudioPlayerProps) {
  return (
    <audio controls src={src} title={title} className="w-[300px]">
      <track kind="captions" />
    </audio>
  );
}
...
function AyahTable({ ayahs }: AyahTableProps) {
  return (
        ...
        <tbody className="divide-y divide-gray-200 bg-white">
          {ayahs.map((ayah) => (
            <tr key={ayah.id} className="hover:bg-gray-50">
              ...
              <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                {ayah.matches === null ? "-" : ayah.matches.toString()}
              </td>
              <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
              ...
              <td className="whitespace-nowrap px-6 py-4 text-sm">
                <LoadingButton
                  ...
                  className="rounded bg-blue-500 px-3 py-1.5 text-white hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                >
                  Edit
                </LoadingButton>
              </td>
...
export default function App() {
  const { ayahs, isLoading, error } = useAyahs();
  ...
  const filteredAyahs = ayahs.filter(
    (ayah) => getSurahNumber(ayah.id) === selectedSurah
  );
